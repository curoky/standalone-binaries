...
**base：** `pkgsStatic.callPackage ./python/<ver>`（见 [local.nix](file:///workspace/standalone-binaries/packages/local.nix#L143-L147)），
即对上游 musl-static `pythonNNN` 做 `overrideAttrs`，全部 **Linux-only**。
...
编译修复**，musl 或 CPython 修好后应回归（见 [regress skill](file:///workspace/standalone-binaries/.trae/skills/regress-patched-package-to-upstream/SKILL.md)）。
...
- **darwin 分支**：`exec -a "$0" python3 "$root/bin/_<name>_main.py"`，回退宿主 `python3`
  （macOS 不 ship 静态解释器）。
