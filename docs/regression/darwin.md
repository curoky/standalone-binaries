...
| `smartmontools` | 🩹 本地 | 🟡 | 目前仅支持 Darwin。`smartctl` 默认内嵌 Nix store 下的外部 drive DB 路径；关闭外部 DB、将可选 local DB 路径设为 `/etc`，继续使用编译期内嵌 DB。build-time `autoreconfHook` 和 `hostname` 会拉入失败的 static Perl，仅将两者换成 native | static Perl 修复后删除 build-tool override；CLI portability 配置保留 | dc5d91f84032 | `packages/smartmontools/darwin.nix`, `packages/local/darwin.nix` |
