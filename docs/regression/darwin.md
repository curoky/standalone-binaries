...
| `gost` | 🩹 本地 | 🟡 | 仅 darwin 有定制（Linux 走零定制 manifest pkgsStatic 全静态）；native Go 构建的 CGO net resolver 拉入 nix-store libresolv stub，postInstall 用 `install_name_tool` 改指 `/usr/lib/libresolv.9.dylib`，standalone 产物只链系统 dylib | stock native 只链接系统 dylib，或 `pkgsStatic` 可直接构建后删除 override | dc5d91f84032 | `packages/gost/` |
